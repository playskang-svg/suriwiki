# 수리위키(SooriWiki) v2.3 — 통합 웹사이트 빌더 & 운영 플랫폼

수리위키(SooriWiki)는 현장 전문가 팀의 실제 시공 데이터를 기반으로 **지역×공정 키워드 페이지(964개+)를 자동 생성**하고, 키워드 단위로 사이트를 쪼개어 **팀·업체에 독립 분양**하는 웹사이트 빌더 및 운영 플랫폼입니다.
