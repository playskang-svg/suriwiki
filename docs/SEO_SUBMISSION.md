# 수리위키(SooriWiki) 네이버 서치어드바이저 & 구글 서치콘솔 색인 등록 가이드
...
2. **웹마스터 도구** &rarr; 대표 도메인 `https://sooriwiki.com` 입력.
...
2. **도메인 속성** 선택 후 `sooriwiki.com` 입력.
...
| **Robots txt** | `/consult`, `/admin`, `/search` 수집 차단 | `https://sooriwiki.com/robots.txt` 접속 |
