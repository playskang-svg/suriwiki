...
export default function ContactBanner({ imageSrc, phone, regionLabel, keyword }: ContactBannerProps) {
  const telHref = `tel:${phone.replace(/[^0-9]/g, '')}`
  return (
    <a
      href={telHref}
      className="group relative my-8 block aspect-[1200/630] w-full overflow-hidden rounded-2xl border border-slate-200 shadow-sm transition-shadow hover:shadow-md"
      aria-label={`${regionLabel} ${keyword} 무료 상담 전화 ${phone}`}
    >
      ...
      <span className="absolute inset-x-0 bottom-0 flex justify-center bg-gradient-to-t from-black/60 via-black/10 to-transparent px-4 pb-5 pt-10">
        <span className="rounded-full bg-cta px-6 py-2.5 text-sm font-bold text-white shadow-lg md:text-base">
          지금 클릭하고 무료 상담받기 →
        </span>
      </span>
    </a>
