...
import { MAIN_CATEGORIES, REGIONS_DATA, generateDefaultBodyContent } from "@/lib/store";
...
export default function AdminKeywordsPage() {
  ...
  // 현재 저장된 내용을 지우지 않고, 자동 생성 원문을 편집창에 불러와서 그 위에서 고칠 수 있게 한다.
  // 저장 버튼을 눌러야만 실제로 반영되므로 실수로 기존 글을 날릴 위험은 없다.
  const handleLoadAutoGenerated = () => {
    const target = pages.find((p) => p.id === editingPageId);
    if (!target) return;

    const cat = MAIN_CATEGORIES.find((c) => c.slug === target.categorySlug);
    const reg = REGIONS_DATA.find((r) => r.slug === target.regionSlug);
    const autoText = generateDefaultBodyContent(cat?.name || target.categorySlug, reg?.name || target.regionSlug);

    if (editBodyContent && editBodyContent !== autoText) {
      if (!confirm("편집창의 현재 내용을 자동 생성 원문으로 덮어씁니다. 계속할까요? (저장 버튼을 누르기 전까지는 반영되지 않습니다.)")) {
        return;
      }
    }
    setEditBodyContent(autoText);
  };
        ...
        <form onSubmit={handleSavePageContent} className="space-y-4 text-xs">
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              <button
                type="button"
                onClick={handleLoadAutoGenerated}
                disabled={!editingPageId}
                className="px-2.5 py-1 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-800 dark:text-slate-200 rounded text-[11px] font-semibold transition disabled:opacity-50"
              >
                🔄 자동 생성 글 불러오기
              </button>
            </div>
            <p className="text-[11px] text-slate-500 mb-1">
              페이지를 선택하면 현재 저장된 본문이 자동으로 편집창에 채워집니다. 원래의 자동 생성 원문으로
              되돌리고 싶다면 위 버튼을 누르세요 — 저장 버튼을 눌러야 실제로 반영됩니다.
            </p>
            <textarea
              rows={8}
