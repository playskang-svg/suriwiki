...
export async function POST(request: Request) {
  try {
    ...
    const { section, title, url, alt, isWatermarked } = body;
...
export async function PUT(request: Request) {
  try {
    const body = await request.json();
    const { id, title, url, alt, isWatermarked, section } = body;

    if (!id) {
      return NextResponse.json({ success: false, message: "이미지 ID가 필요합니다." }, { status: 400 });
    }

    const updated = updateSiteImage(id, { title, url, alt, isWatermarked, section });
    if (!updated) {
